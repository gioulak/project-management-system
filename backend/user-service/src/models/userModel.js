...
class User {
  ...
  static async findByUsername(username) {
    const [rows] = await db.query(
      'SELECT id, username, email, first_name, last_name, role, is_active, password FROM users WHERE username = ?',
      [username]
    );
    return rows[0];
  }
  ...
  static async findByEmail(email) {
    const [rows] = await db.query(
      'SELECT id, username, email, first_name, last_name, role, is_active FROM users WHERE email = ?',
      [email]
    );
    return rows[0];
  }
